...
布线方案管理支持按关键词、启用状态筛选，并可“导出当前筛选结果”为 CSV（UTF-8 BOM，Excel 可直接打开）。导出列为方案名称、适用场景、启用状态、配件名称、所属分区、需求数量、规格单位；同一方案的配件按库房分区排序号排列，方案信息逐行重复以保留方案边界；空结果、超长名称、中文文件名与导出中的重复点击均有明确处理（接口 `GET /api/wiring-plan/export`）。
