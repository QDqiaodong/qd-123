// 现存量变更通知：配件档案保存（新增/编辑）现存量、分区调整、方案核销出库、分区盘点确认回写后，
// 布线方案列表的库存校验、库存缺口分析与分区盘点列表都依赖同一现存量实时重算，
// 仅刷新当前页会让其他已打开页面停留在旧的“充足/不足”与缺口数字上，故统一在此广播，
// 各页面监听到后立即按最新现存量重新拉取。
// 补货单提交/作废不改现存量，但会改变配件档案与安全库存台账的“待补”标记与勾选可用性，
// 同样通过该广播让配件档案、台账、补货单列表立即刷新。
